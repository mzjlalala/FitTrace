...
| 训练记录 | POST/GET `/api/training-records` | 创建 / 历史列表（分页 + 日期筛选） |
| 训练记录 | GET/PUT/DELETE `/api/training-records/{id}` | 详情（含组数据）/ 整体更新 / 删除 |
| 统计 | GET `/api/training/stats/summary` | 总次数/总时长/打卡天数/连续打卡/各动作 PR |
| 统计 | GET `/api/training/stats/heatmap` | 近 365 天每日训练条数（热力图数据） |
...
- **打卡天数**：按训练日期去重，当天 ≥1 条记录即打卡 1 次
- **连续打卡**：从今天往回数连续有记录的天数（今天无记录则从昨天开始数）
- **PR（个人纪录）**：每个动作所有**已完成（done_flag=true）且重量 >0** 的组中，重量最大者（同重量取次数多、再取日期近）
- **热力图**：近 365 天每天记录条数，无记录为 0
- **食物营养换算**：单笔摄入 = 食物每 100g 营养 × 食用克数 ÷ 100（保留 1 位）；每日汇总 = 当天所有记录之和
